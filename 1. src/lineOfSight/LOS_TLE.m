...
% Read parameters from satelliteParameters.txt
params = readtable(fullfile('..', 'networkSetup', 'satelliteParameters.txt'), 'Delimiter', ':', 'ReadVariableNames', false);
params = table2struct(params, 'ToScalar', true);
...
% Write the combined tables to Excel files in the specified output directory
writetable(allIntervals, fullfile(outputDir, 'AccessIntervals.xlsx'));
writetable(allLookAngles, fullfile(outputDir, 'LookAngles.xlsx'));
